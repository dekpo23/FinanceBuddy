...
class PortfolioRecommendation(Base):
    """
    Stores portfolio recommendations generated by the Trading Agent.
    """
    __tablename__ = "portfolio_recommendations"

    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("users.id"))
    portfolio = Column(String, nullable=False) # JSON list of assets/weights
    risk_level = Column(String, nullable=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now())

    user = relationship("User", back_populates="recommendations")

User.recommendations = relationship("PortfolioRecommendation", back_populates="user")
